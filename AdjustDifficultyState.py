...
# Provides the UI for adjusting difficulty settings, including paddle width,
# paddle movement speed, and ball movement speed. The slider values are saved to the GameData.
class AdjustDifficultyState(GameState):
    # Initialize the AdjustDifficultyState instance.
    def __init__(self):
        ...
        # Create a vertical layout to hold the slider groups.
        self.slider_box = arcade.gui.UIBoxLayout(vertical=True, space_between=20)

        # --- Paddle Width Slider ---
        # Label and slider for adjusting paddle width.
        ...
        self.paddle_width_slider = UISlider(
            value=GameData().difficulty_paddleWidth, width=300, height=50,
            min_value=1, max_value=10, step=1
        )
        paddle_width_value_label = arcade.gui.UILabel(
            text=f"{int(self.paddle_width_slider.value)}", font_size=16,
            text_color=arcade.color.WHITE
        )
        @self.paddle_width_slider.event("on_change")
        def on_change_pw(event: UIOnChangeEvent):
            # Update label and pass new paddle width value to GameData.
        ...
        # Label and slider for adjusting paddle movement speed.
        ...
        self.paddle_speed_slider = UISlider(
            value=GameData().difficulty_paddleMoveSpeed, width=300, height=50,
            min_value=1, max_value=10, step=1
        )
        paddle_speed_value_label = arcade.gui.UILabel(
            text=f"{int(self.paddle_speed_slider.value)}", font_size=16,
            text_color=arcade.color.WHITE
        )
        @self.paddle_speed_slider.event("on_change")
        def on_change_ps(event: UIOnChangeEvent):
            # Update label and pass new paddle movement speed value to GameData.
            paddle_speed_value_label.text = f"{int(self.paddle_speed_slider.value)}"
            paddle_speed_value_label.fit_content()
            GameData().setPaddleMoveSpeedDifficulty(int(self.paddle_speed_slider.value))
        ...
        # Label and slider for adjusting ball movement speed.
        ...
        self.ball_speed_slider = UISlider(
            value=GameData().difficulty_ballSpeed, width=300, height=50,
            min_value=1, max_value=10, step=1
        )
        ball_speed_value_label = arcade.gui.UILabel(
            text=f"{int(self.ball_speed_slider.value)}", font_size=16,
            text_color=arcade.color.WHITE
        )
        @self.ball_speed_slider.event("on_change")
        def on_change_bs(event: UIOnChangeEvent):
            # Update label and pass new ball speed value to GameData.
            ball_speed_value_label.text = f"{int(self.ball_speed_slider.value)}"
            ball_speed_value_label.fit_content()
            GameData().setBallSpeedDifficulty(int(self.ball_speed_slider.value))
    ...
    # Update the state (no dynamic updates required).
    def updateState(self):
        pass

    # Draw the Adjust Difficulty UI.
    def drawState(self):
        self.manager.draw()
